...
<html>
    ...
    <body>
        ...
        <form action="allocate_projectsql.php" onsubmit="return Validate()" method="post">
            <h3 class="msg">
                <?php
                if(isset($_GET['msg']))
                {
            ...
            <table align="center" border="1">
                <tr>
                    <th colspan="2">Allocate Project</th>
                </tr>
            <?php
            try{
                ...
                $query=$dbhandler->prepare("select * from groups where leader=?");
                $query->execute(array($_GET['leader']));
                $r=$query->fetch();
                if($r==null)
                {
                    ...
                    echo '<tr>
                    <td>Leader Id</td>
                    <td><input type="hidden" id="id" name="p_id" value="'.$r['leader'].'">'.$r['leader'].'</td>
                </tr>
                <tr>
                    <td>PID to allocate</td>
                    <td><input type="text" id="pidal" class="textbox" name="p_definition" value="'.$r['pidal'].'" required></td>    
                </tr>';                ;
                }
            }
            ...
            ?>
                <tr>
                    <td colspan="2">&emsp;&emsp;<input type="submit" class="button shadow" value="Allocate PID">&emsp;&emsp;&emsp;
